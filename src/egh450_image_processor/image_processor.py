...
import uavservo17BLUE
import uavservo18ORANGE
...
class ImageProcessor():
	def __init__(self):
		...
		if self.param_use_compressed:
			...
			#self.pub_mask_triangle = rospy.Publisher("~debug/image/compressed_triangle", CompressedImage, queue_size=1)
			...
			self.pub_mask_square = rospy.Publisher("~debug/image_raw_square", Image, queue_size=1)
			self.pub_overlay = rospy.Publisher("~overlay/image_raw", Image, queue_size=1)

		self.tfbr = tf2_ros.TransformBroadcaster()
                                self.sub_depl = rospy.Subscriber('/UAVUSR_G9/deployed', Bool, self.callback_deploy)
	...
	def process_image(self, cv_image, process_type):
		...
		if process_type == 2: #Triangle

			hue_lower_triangle = (self.param_hue_center_triangle - self.param_hue_range_triangle) % 180
			hue_upper_triangle = (self.param_hue_center_triangle + self.param_hue_range_triangle) % 180

			thresh_lower_triangle = np.array([hue_lower_triangle, self.param_val_min_triangle, self.param_val_min_triangle])
			thresh_upper_triangle = np.array([hue_upper_triangle, self.param_val_max_triangle, self.param_val_max_triangle])


			if hue_lower_triangle > hue_upper_triangle:
				# We need to do a wrap around HSV 180 to 0 if the user wants to mask this color
				thresh_lower_wrap_triangle = np.array([180, self.param_sat_max_triangle, self.param_val_max_triangle])
				thresh_upper_wrap_triangle = np.array([0, self.param_sat_min_triangle, self.param_val_min_triangle])
				...
				mask_image = cv2.bitwise_or(mask_lower_triangle, mask_upper_triangle)
			else:
				# Otherwise do a simple mask
				mask_image = cv2.inRange(hsv_image, thresh_lower_triangle, thresh_upper_triangle)
